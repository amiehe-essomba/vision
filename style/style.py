...
class config:
    ...
    def main(self, char : str = "=", language : str = "python", cmt : str = "", cc : str = "", color: str = "", COLOR : dict = {}):
        ...
        if char and char != cmt:
            if   char in {'<', '>', '=', '!', '|', '&', '?', "~"}   : self.return_char =  config(self.termios).operators(char=char, language=language, cc=cc, COLOR=COLOR.copy())
            elif char in {'+', '-', '*', '^', '%', '/'}             : self.return_char =  config(self.termios).arithmetic(char=char, language=language,cc=cc, COLOR=COLOR.copy())
            elif char in {"'", '"'}                                 : self.return_char =  config(self.termios).quote(char=char, language=language,cc=cc, color=color, COLOR=COLOR.copy())
            elif char in {str(x) for x in range(10)}                : self.return_char =  config(self.termios).number(char=char, language=language,cc=cc, COLOR=COLOR.copy())
            elif char in {"[","]",'{','}', "(", ")"}                : self.return_char =  config(self.termios).bracket(char=char, language=language,cc=cc, COLOR=COLOR.copy())
            ...
            else                                                    : self.return_char =  config(self.termios).rest(char=char, language=language,cc=cc, COLOR=COLOR.copy())
    ...
    def operators(self, char: str = "=", language : str = "mamba", cc : str = "", COLOR : dict = {})      :
        # initialization 
        self.text = ""
        
        if language in ['python', 'mamba',  "c", "c++", "cpmd", "cython"] : 
            # default text color white 
            self.text = COLOR['op'] + char + self.init.reset
        else: pass 
        
        return self.text
    
    def arithmetic(self, char: str = "+", language : str = "mamba", cc : str = "", COLOR : dict = {})     :
        # initialization 
        self.text = ""
        
        if language in ['python', 'mamba',  "c", "c++", "cpmd", "cython"] : 
            # default text color white 
            self.text = COLOR['ar'] + char + self.init.reset
        else: pass 
    ...
    def number(self, char: str = "1", language : str = "mamba", cc : str = "", COLOR : dict = {})         :
        # initialization 
        self.text = ""
        
        if language in ['python', 'mamba', "c", "c++", "cpmd", "cython"] : 
            # default text color white 
            self.text = COLOR['num'] + char + self.init.reset
        else: pass 
    ...
    def bracket(self, char: str = "(", language : str = "mamba", cc : str = "", COLOR : dict = {})        :
        # initialization 
        self.text = ""
        
        if language in ['python', 'mamba',"c", "c++", "cpmd", "cython"] : 
            # default text color white 
            if char in ['(', ')']: self.text = COLOR['bracket']['('] + char + self.init.reset  
            if char in ['[', ']']: self.text = COLOR['bracket']['['] + char + self.init.reset  
            if char in ['{', '}']: self.text = COLOR['bracket']['{'] + char + self.init.reset  
        else: pass 
    ...
    def rest(self, char: str = ":", language : str = "mamba", cc : str = "", COLOR : dict ={})           :
        # initialization 
        self.text = ""
